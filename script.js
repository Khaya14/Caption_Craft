const apiKey = "AIzaSyBdSzebjq035gfTa6dQStMyTA4zi11NFxs";
...
function buildApiPayload(inputs) {
  ...
  const userQuery = `Topic: ${inputs.topic}. Tone: ${
    inputs.tone
  }. Constraints: ${inputs.constraint || "None specified."}.`;

  const responseSchema = {
    type: "ARRAY",
    description:
      "An array of generated social media captions, one for each platform.",
    items: {
      type: "OBJECT",
      properties: {
        platform: {
          type: "STRING",
          description:
            "The social media platform (e.g., 'Instagram', 'Twitter', 'LinkedIn').",
        },
        caption: {
          type: "STRING",
...
async function generateCaptions() {
  ...
  if (!inputs.topic) {
    outputArea.innerHTML =
      '<p class="error-message">Please enter a **Topic** to generate captions.</p>';
    return;
  }
  ...
  outputArea.innerHTML =
    '<div class="spinner"></div><p style="font-style: italic; margin-top: 10px;">Crafting captions...</p>';
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("outputArea").innerHTML =
    '<p>Enter your topic and click "Generate" to begin.</p>';
});
